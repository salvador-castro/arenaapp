...
// lo que devuelve la API admin de cafes
interface AdminCafe {
  id: number | string
  nombre: string
...
export default function CafesPage() {
  ...
  const [editing, setEditing] = useState<AdminCafe | null>(null)
  ...
  const [deleteTarget, setDeleteTarget] = useState<AdminCafe | null>(null)
  ...
  useEffect(() => {
    if (isLoading) return
    if (!user) {
      router.push('/login?redirect=/cafes')
      return
    }
    if (user.rol !== 'ADMIN') {
  ...
  async function fetchCafes(pageToLoad: number, searchTerm: string) {
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/admin/cafes?${params.toString()}`,
        {
          method: 'GET',
          credentials: 'include',
        }
      ...
      if (!res.ok) {
        throw new Error(`Error al cargar cafés (${res.status})`)
      }

      const json = await res.json()

      const data: AdminCafe[] = (json.data as any[]).map((row) => ({
        ...row,
        rango_precios:
      ...
      console.error(err)
      setError(err.message ?? 'Error al cargar cafés')
    } finally {
      setIsFetching(false)
    }
  }
  ...
  function openEditForm(c: AdminCafe) {
    setEditing(c)
    setFormValues({
      nombre: c.nombre ?? '',
      ...
      pais: c.pais ?? '',
      url_maps: c.url_maps ?? '',
      horario_text: c.horario_text ?? '',
      url_reserva: c.url_reserva ?? '',
      instagram: c.instagram ?? '',
      sitio_web: c.sitio_web ?? '',
      url_imagen: c.url_imagen ?? '',
      es_destacado: !!c.es_destacado,
      resena: c.resena ?? '',
      estado: (c.estado as FormValues['estado']) ?? 'PUBLICADO',
    })
    setIsFormOpen(true)
  }
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const url = isEdit
        ? `${API_BASE}/api/admin/cafes/${idForUrl}`
        : `${API_BASE}/api/admin/cafes`
      ...
      if (!res.ok) {
        ...
        throw new Error(msg || `Error al guardar café (${res.status})`)
      }

      await res.json() // no lo usamos, solo aseguramos parseo OK

      await fetchCafes(currentPage, search)
      closeForm()
    } catch (err: any) {
      console.error(err)
      setError(err.message ?? 'Error al guardar café')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  async function confirmDelete() {
    ...
    try {
      const url = `${API_BASE}/api/admin/cafes/${String(deleteTarget.id)}`
      ...
      if (!res.ok) {
        const msg = await res.text()
        throw new Error(msg || `Error al eliminar café (${res.status})`)
      }
      ...
      console.error(err)
      setError(err.message ?? 'Error al eliminar café')
    } finally {
      setIsDeleting(false)
    }
  }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
      ...
      <header className="sticky top-0 z-40 bg-slate-950/90 backdrop-blur border-b border-slate-800">
        <div className="max-w-4xl mx-auto flex items-center justify-between px-4 py-3">
          <div>
            <h1 className="text-lg font-semibold">Gestión de cafés</h1>
            <p className="text-xs text-slate-400">
              Crear, editar y eliminar cafés de ArenaApp.
            </p>
          </div>
          <UserDropdown />
        </div>
      ...
      <main className="max-w-4xl mx-auto px-4 pt-4 pb-6">
        ...
        {isFetching && (
          <div className="mb-3 text-xs text-slate-400">Cargando cafés...</div>
        )}
              ...
              <tbody>
                ...
                {cafes.map((c) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-xs text-slate-300">
                      {priceTierToSymbols(c.rango_precios ?? null)}
                    </td>
                    <td className="px-3 py-2 text-xs text-yellow-300">
                      {c.estrellas ? '★'.repeat(Math.min(c.estrellas, 5)) : '-'}
                    </td>
                    <td className="px-3 py-2 text-xs text-slate-300">
                      {c.es_destacado ? 'Si' : 'No'}
                    </td>
            ...
            <div className="w-full max-w-2xl mx-auto my-8 rounded-3xl bg-slate-950 border border-slate-700 p-6 md:p-8 shadow-2xl max-h-[88vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold">
                  {editing ? 'Editar café' : 'Nuevo café'}
                </h2>
                <button
                  type="button"
                  onClick={closeForm}
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    required
                    className="w-full rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                    placeholder="Escribí una reseña descriptiva del café..."
                  />
                  <p className="mt-1 text-[11px] text-slate-500">
                    Podés escribir un texto largo: ambiente, especialidades,
                    recomendaciones, etc.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="horario_text"
                    placeholder="Lun a vie de 8 a 20, sáb y dom de 9 a 18..."
                ...
                <div>
                  ...
                  <p className="mt-1 text-[10px] text-slate-500">
                    ...
                    <code>uploads/cafes/archivo.jpg</code>).
                  </p>
                </div>
        ...
        {deleteTarget && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
            <div className="w-full max-w-sm rounded-2xl bg-slate-950 border border-slate-700 p-4 shadow-2xl">
              <h2 className="text-sm font-semibold mb-2">Eliminar café</h2>
