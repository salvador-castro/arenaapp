///Users/salvacastro/Desktop/arenaapp/arenaapp-front/src/components/destacados/Hoteles.tsx
'use client'
...
export default function HotelesDestacados({ isLoggedIn }: Props) {
  ...
  // const userRole: string | undefined = auth?.user?.role // si después lo necesitás
  ...
  const t = {
    es: {
      ...
      error: 'No se pudieron cargar los hoteles destacados.',
      empty: 'Cuando el admin cargue hoteles, los vas a ver listados acá.',
      locationFallback: 'Ubicación no especificada',
      priceFromNight: 'Desde {price} {currency} / noche',
      seeMore: 'Ver más',
      reviewTitle: 'Reseña',
    ...
    en: {
      ...
      error: 'Could not load featured hotels.',
      empty: 'Once the admin adds hotels, you will see them listed here.',
      locationFallback: 'Location not specified',
      priceFromNight: 'From {price} {currency} / night',
      seeMore: 'See more',
      reviewTitle: 'Review',
    ...
    pt: {
      ...
      error: 'Não foi possível carregar os hotéis em destaque.',
      empty: 'Quando o admin cadastrar hotéis, você verá a lista aqui.',
      locationFallback: 'Localização não especificada',
      priceFromNight: 'Desde {price} {currency} / noite',
      seeMore: 'Ver mais',
      reviewTitle: 'Resenha',
  ...
  useEffect(() => {
    const fetchDestacados = async () => {
      try {
        ...
        const hoteles: Hotel[] = Array.isArray(data)
          ? data
          : (data.hoteles ?? [])
                  ...
                  {typeof place.precio_noche_desde === 'number' &&
                    place.precio_noche_desde > 0 && (
                      <span className="inline-flex rounded-full border border-slate-700 px-2 py-[2px] text-[10px] text-slate-300">
                        {t.priceFromNight
                          .replace('{price}', String(place.precio_noche_desde))
                          .replace('{currency}', place.moneda || 'ARS')}
                      </span>
                    )}
                </div>
                      ...
                      selectedPlace.precio_noche_desde > 0 && (
                        <span className="inline-flex rounded-full border border-slate-700 px-2 py-[2px] text-[10px] text-slate-300">
                          {t.priceFromNight
                            .replace(
                              '{price}',
                              String(selectedPlace.precio_noche_desde)
                            )
                            .replace(
                              '{currency}',
                              selectedPlace.moneda || 'ARS'
                            )}
                        </span>
                      )}
                  </div>
